...
contract PoolBetting is Ownable {
    ...
    function createBettingPool(
        ...
        require(
            timeToDeadline > block.timestamp,
            "Deadline must be in the future"
        );
        require(
            supportedTokens.length > 0,
            "At least one supported token required"
        );
        ...
        newPool.deadline = block.timestamp + timeToDeadline;
    ...
    function placeTokenBet(
        ...
        require(
            ...
            "Betting pool deadline has passed"
        );
    ...
    function userInPool(uint256 poolId, address user)
        internal
        view
        returns (bool)
    {
    ...
    function setOutcome(uint256 poolId, string memory _outcome)
        public
        onlyOwner
    {
        BettingPool storage pool = allPools[poolId];
    ...
    function distributeRewards(uint256 poolId) public onlyOwner {
        ...
        for (uint256 i = 0; i < pool.users.length; i++) {
            ...
            if (
                keccak256(abi.encodePacked(userBet.answer)) ==
                keccak256(abi.encodePacked(pool.outcome))
            ) {
                totalWinnerBet += calculateUserTotalBet(
                    userBet,
        ...
        for (uint256 i = 0; i < pool.supportedTokens.length; i++) {
            ...
            for (uint256 j = 0; j < pool.users.length; j++) {
                ...
                if (
                    ...
                    uint256 userTotalBet = calculateUserTotalBet(
                        userBet,
                        pool.supportedTokens
                    );
                    uint256 userShare = (userTotalBet * tokenPoolAmount) /
                        totalWinnerBet;
    ...
    function calculateTotalPoolSize(uint256 poolId)
        internal
        view
        returns (uint256)
    {
    ...
    function calculateUserTotalBet(
        UserBet storage userBet,
        address[] memory supportedTokens
    ...
    function distributeFee(
        ...
        for (uint256 i = 0; i < supportedTokens.length; i++) {
            ...
            uint256 feeTokenAmount = (feeAmount * tokenBalance) /
                calculateTotalPoolSize(poolId);
    ...
    function distributeTokensToWinner(
        BettingPool storage pool,
        address winner,
        uint256 userShare,
        uint256 randNonce
    ) internal {
        ...
        for (uint256 i = 0; i < tokenCount; i++) {
            ...
            uint256 tokenBalance = IERC20(token).balanceOf(address(this));
            uint256 userTokenShare = (userShare * tokenBalance) /
                calculateTotalPoolSize(pool.poolId);
    ...
    function getPoolBettors(uint256 poolId)
        public
        view
        returns (
            address[] memory,
    ...
    function getUserBets(uint256 poolId, address userAddress)
        public
        view
